...
var require_localforage = /* @__PURE__ */ __commonJS({ "node_modules/localforage/dist/localforage.js": ((exports, module) => {
	(function(f) {
		...
		return (function e(t, n, r) {
			...
			3: [function(_dereq_, module$1, exports$1) {
				(function(global$1) {
					if (typeof global$1.Promise !== "function") global$1.Promise = _dereq_(2);
				}).call(this, typeof global !== "undefined" ? global : typeof self !== "undefined" ? self : typeof window !== "undefined" ? window : {});
			}, { "2": 2 }],
			4: [function(_dereq_, module$1, exports$1) {
				...
				function normalizeKey(key$3) {
					if (typeof key$3 !== "string") {
						console.warn(key$3 + " used as a key, but it is not a string.");
						key$3 = String(key$3);
					}
					return key$3;
				}
				...
				function _binStringToArrayBuffer(bin) {
					var length$3 = bin.length;
					var buf = new ArrayBuffer(length$3);
					var arr = new Uint8Array(buf);
					for (var i = 0; i < length$3; i++) arr[i] = bin.charCodeAt(i);
					return buf;
				}
				...
				function _fullyReady(callback) {
					var self$1 = this;
					var promise = self$1._initReady().then(function() {
						var dbContext = dbContexts[self$1._dbInfo.name];
				...
				function _tryReconnect(dbInfo) {
					...
					return _getOriginalConnection(dbInfo).then(function(db) {
						...
						for (var i$1 = 0; i$1 < forages.length; i$1++) forages[i$1]._dbInfo.db = db;
					})["catch"](function(err) {
				...
				function _initStorage(options) {
					...
					dbContext.forages.push(self$1);
					if (!self$1._initReady) {
						self$1._initReady = self$1.ready;
						self$1.ready = _fullyReady;
					}
					var initPromises = [];
					...
					for (var j = 0; j < dbContext.forages.length; j++) {
						...
						if (forage !== self$1) initPromises.push(forage._initReady()["catch"](ignoreErrors));
					}
					...
					return Promise$1.all(initPromises).then(function() {
						...
						dbInfo.db = db;
						if (_isUpgradeNeeded(dbInfo, self$1._defaultConfig.version)) return _getUpgradedConnection(dbInfo);
						return db;
					}).then(function(db) {
						dbInfo.db = dbContext.db = db;
						self$1._dbInfo = dbInfo;
						for (var k = 0; k < forages.length; k++) {
							var forage$1 = forages[k];
							if (forage$1 !== self$1) {
								forage$1._dbInfo.db = dbInfo.db;
								forage$1._dbInfo.version = dbInfo.version;
							}
						}
					});
				}
				function getItem(key$3, callback) {
					var self$1 = this;
					key$3 = normalizeKey(key$3);
					var promise = new Promise$1(function(resolve, reject) {
						self$1.ready().then(function() {
							createTransaction(self$1._dbInfo, READ_ONLY, function(err, transaction) {
								if (err) return reject(err);
								try {
									var req = transaction.objectStore(self$1._dbInfo.storeName).get(key$3);
					...
					var promise = new Promise$1(function(resolve, reject) {
						self$1.ready().then(function() {
							createTransaction(self$1._dbInfo, READ_ONLY, function(err, transaction) {
								if (err) return reject(err);
								try {
									var req = transaction.objectStore(self$1._dbInfo.storeName).openCursor();
				...
				function setItem(key$3, value, callback) {
					var self$1 = this;
					key$3 = normalizeKey(key$3);
					var promise = new Promise$1(function(resolve, reject) {
						var dbInfo;
						self$1.ready().then(function() {
							dbInfo = self$1._dbInfo;
							...
							createTransaction(self$1._dbInfo, READ_WRITE, function(err, transaction) {
								if (err) return reject(err);
								try {
									var store = transaction.objectStore(self$1._dbInfo.storeName);
									if (value$1 === null) value$1 = void 0;
									var req = store.put(value$1, key$3);
				...
				function removeItem(key$3, callback) {
					var self$1 = this;
					key$3 = normalizeKey(key$3);
					var promise = new Promise$1(function(resolve, reject) {
						self$1.ready().then(function() {
							createTransaction(self$1._dbInfo, READ_WRITE, function(err, transaction) {
								if (err) return reject(err);
								try {
									var req = transaction.objectStore(self$1._dbInfo.storeName)["delete"](key$3);
									transaction.oncomplete = function() {
										resolve();
									};
					...
					var promise = new Promise$1(function(resolve, reject) {
						self$1.ready().then(function() {
							createTransaction(self$1._dbInfo, READ_WRITE, function(err, transaction) {
								if (err) return reject(err);
								try {
									var req = transaction.objectStore(self$1._dbInfo.storeName).clear();
									transaction.oncomplete = function() {
										resolve();
									};
									transaction.onabort = transaction.onerror = function() {
					...
					var promise = new Promise$1(function(resolve, reject) {
						self$1.ready().then(function() {
							createTransaction(self$1._dbInfo, READ_ONLY, function(err, transaction) {
								if (err) return reject(err);
								try {
									var req = transaction.objectStore(self$1._dbInfo.storeName).count();
									req.onsuccess = function() {
										resolve(req.result);
									};
									req.onerror = function() {
					...
					var promise = new Promise$1(function(resolve, reject) {
						self$1.ready().then(function() {
							createTransaction(self$1._dbInfo, READ_ONLY, function(err, transaction) {
								if (err) return reject(err);
								try {
									var req = transaction.objectStore(self$1._dbInfo.storeName).openKeyCursor();
									var keys$3 = [];
									req.onsuccess = function() {
										var cursor = req.result;
										if (!cursor) {
				...
				function dropInstance(options, callback) {
					...
					if (!options.name) promise = Promise$1.reject("Invalid arguments");
						...
						var dbPromise = options.name === currentConfig.name && self$1._dbInfo.db ? Promise$1.resolve(self$1._dbInfo.db) : _getOriginalConnection(options).then(function(db) {
						...
						if (!options.storeName) promise = dbPromise.then(function(db) {
							...
							return new Promise$1(function(resolve, reject) {
								...
								dbContext.db = db$1;
								for (var i$1 = 0; i$1 < forages.length; i$1++) {
									var _forage = forages[i$1];
				...
				function tryExecuteSql(t, dbInfo, sqlStatement, args, callback, errorCallback) {
					t.executeSql(sqlStatement, args, callback, function(t$1, error) {
						if (error.code === error.SYNTAX_ERR) t$1.executeSql("SELECT name FROM sqlite_master WHERE type='table' AND name = ?", [dbInfo.storeName], function(t$2, results) {
							if (!results.rows.length) createDbTable(t$2, dbInfo, function() {
								t$2.executeSql(sqlStatement, args, callback, errorCallback);
							}, errorCallback);
							else errorCallback(t$2, error);
						}, errorCallback);
						else errorCallback(t$1, error);
					}, errorCallback);
				}
				function getItem$1(key$3, callback) {
					var self$1 = this;
					key$3 = normalizeKey(key$3);
					var promise = new Promise$1(function(resolve, reject) {
						self$1.ready().then(function() {
							...
							dbInfo.db.transaction(function(t) {
								tryExecuteSql(t, dbInfo, "SELECT * FROM " + dbInfo.storeName + " WHERE key = ? LIMIT 1", [key$3], function(t$1, results) {
				...
				function iterate$1(iterator, callback) {
					...
					var promise = new Promise$1(function(resolve, reject) {
						self$1.ready().then(function() {
							var dbInfo = self$1._dbInfo;
							dbInfo.db.transaction(function(t) {
								tryExecuteSql(t, dbInfo, "SELECT * FROM " + dbInfo.storeName, [], function(t$1, results) {
				...
				function _setItem(key$3, value, callback, retriesLeft) {
					var self$1 = this;
					key$3 = normalizeKey(key$3);
					var promise = new Promise$1(function(resolve, reject) {
						self$1.ready().then(function() {
							...
							var dbInfo = self$1._dbInfo;
							dbInfo.serializer.serialize(value, function(value$1, error) {
								if (error) reject(error);
								else dbInfo.db.transaction(function(t) {
									tryExecuteSql(t, dbInfo, "INSERT OR REPLACE INTO " + dbInfo.storeName + " (key, value) VALUES (?, ?)", [key$3, value$1], function() {
										resolve(originalValue);
									}, function(t$1, error$1) {
										reject(error$1);
									});
								}, function(sqlError) {
									if (sqlError.code === sqlError.QUOTA_ERR) {
										if (retriesLeft > 0) {
											resolve(_setItem.apply(self$1, [
												key$3,
												originalValue,
												callback,
												retriesLeft - 1
				...
				function setItem$1(key$3, value, callback) {
				...
				function removeItem$1(key$3, callback) {
					var self$1 = this;
					key$3 = normalizeKey(key$3);
					var promise = new Promise$1(function(resolve, reject) {
						self$1.ready().then(function() {
							var dbInfo = self$1._dbInfo;
							dbInfo.db.transaction(function(t) {
								tryExecuteSql(t, dbInfo, "DELETE FROM " + dbInfo.storeName + " WHERE key = ?", [key$3], function() {
									resolve();
								}, function(t$1, error) {
									reject(error);
								});
				...
				function length$1(callback) {
					...
					var promise = new Promise$1(function(resolve, reject) {
						self$1.ready().then(function() {
							var dbInfo = self$1._dbInfo;
							dbInfo.db.transaction(function(t) {
								tryExecuteSql(t, dbInfo, "SELECT COUNT(key) as c FROM " + dbInfo.storeName, [], function(t$1, results) {
				...
				function key$1(n, callback) {
					...
					var promise = new Promise$1(function(resolve, reject) {
						self$1.ready().then(function() {
							var dbInfo = self$1._dbInfo;
							dbInfo.db.transaction(function(t) {
								tryExecuteSql(t, dbInfo, "SELECT key FROM " + dbInfo.storeName + " WHERE id = ? LIMIT 1", [n + 1], function(t$1, results) {
				...
				function keys$1(callback) {
					...
					var promise = new Promise$1(function(resolve, reject) {
						self$1.ready().then(function() {
							var dbInfo = self$1._dbInfo;
							dbInfo.db.transaction(function(t) {
								tryExecuteSql(t, dbInfo, "SELECT key FROM " + dbInfo.storeName, [], function(t$1, results) {
									var keys$3 = [];
									for (var i = 0; i < results.rows.length; i++) keys$3.push(results.rows.item(i).key);
									resolve(keys$3);
								}, function(t$1, error) {
									reject(error);
								});
				...
				function getAllStoreNames(db) {
					return new Promise$1(function(resolve, reject) {
						db.transaction(function(t) {
							t.executeSql("SELECT name FROM sqlite_master WHERE type='table' AND name <> '__WebKitDatabaseInfoTable__'", [], function(t$1, results) {
								var storeNames = [];
				...
				function dropInstance$1(options, callback) {
					...
					if (!options.name) promise = Promise$1.reject("Invalid arguments");
						...
						var db;
						if (options.name === currentConfig.name) db = self$1._dbInfo.db;
				...
				function _initStorage$2(options) {
					...
					var dbInfo = {};
					if (options) for (var i in options) dbInfo[i] = options[i];
					dbInfo.keyPrefix = _getKeyPrefix(options, self$1._defaultConfig);
				...
				function clear$2(callback) {
					var self$1 = this;
					var promise = self$1.ready().then(function() {
						var keyPrefix = self$1._dbInfo.keyPrefix;
						for (var i = localStorage.length - 1; i >= 0; i--) {
							var key$3 = localStorage.key(i);
							if (key$3.indexOf(keyPrefix) === 0) localStorage.removeItem(key$3);
						}
					});
					executeCallback(promise, callback);
					return promise;
				}
				function getItem$2(key$3, callback) {
					var self$1 = this;
					key$3 = normalizeKey(key$3);
					var promise = self$1.ready().then(function() {
						var dbInfo = self$1._dbInfo;
						var result = localStorage.getItem(dbInfo.keyPrefix + key$3);
						if (result) result = dbInfo.serializer.deserialize(result);
						return result;
					});
					executeCallback(promise, callback);
				...
				function iterate$2(iterator, callback) {
					...
					var promise = self$1.ready().then(function() {
						...
						var length$3 = localStorage.length;
						var iterationNumber = 1;
						for (var i = 0; i < length$3; i++) {
							var key$3 = localStorage.key(i);
							if (key$3.indexOf(keyPrefix) !== 0) continue;
							var value = localStorage.getItem(key$3);
							if (value) value = dbInfo.serializer.deserialize(value);
							value = iterator(value, key$3.substring(keyPrefixLength), iterationNumber++);
				...
				function keys$2(callback) {
					...
					var promise = self$1.ready().then(function() {
						...
						for (var i = 0; i < length$3; i++) {
							...
							if (itemKey.indexOf(dbInfo.keyPrefix) === 0) keys$3.push(itemKey.substring(dbInfo.keyPrefix.length));
						}
						return keys$3;
					});
					executeCallback(promise, callback);
				...
				function length$2(callback) {
					var promise = this.keys().then(function(keys$3) {
						return keys$3.length;
					});
					executeCallback(promise, callback);
					return promise;
				}
				function removeItem$2(key$3, callback) {
					var self$1 = this;
					key$3 = normalizeKey(key$3);
					var promise = self$1.ready().then(function() {
						var dbInfo = self$1._dbInfo;
						localStorage.removeItem(dbInfo.keyPrefix + key$3);
					});
					executeCallback(promise, callback);
					return promise;
				}
				function setItem$2(key$3, value, callback) {
					var self$1 = this;
					key$3 = normalizeKey(key$3);
					var promise = self$1.ready().then(function() {
						if (value === void 0) value = null;
						...
						return new Promise$1(function(resolve, reject) {
							...
							dbInfo.serializer.serialize(value, function(value$1, error) {
								if (error) reject(error);
									...
									localStorage.setItem(dbInfo.keyPrefix + key$3, value$1);
									resolve(originalValue);
								} catch (e) {
				...
				function dropInstance$2(options, callback) {
					...
					if (!options.name) promise = Promise$1.reject("Invalid arguments");
						...
						else resolve(_getKeyPrefix(options, self$1._defaultConfig));
				...
				var sameValue = function sameValue$1(x, y) {
					return x === y || typeof x === "number" && typeof y === "number" && isNaN(x) && isNaN(y);
				};
				var includes = function includes$1(array, searchElement) {
				...
				module$1.exports = new (function() {
						...
						var promise = new Promise$1(function(resolve, reject) {
							try {
								...
								(function configureMissingMethods() {
									var methodNotImplementedFactory = function methodNotImplementedFactory$1(methodName) {
										return function() {
											var error = /* @__PURE__ */ new Error("Method " + methodName + " is not implemented by the current driver");
											var promise$1 = Promise$1.reject(error);
											executeCallback(promise$1, arguments[arguments.length - 1]);
											return promise$1;
										};
									};
								...
								var setDriverSupport = function setDriverSupport$1(support) {
					...
					LocalForage.prototype.ready = function ready(callback) {
						var self$1 = this;
						var promise = self$1._driverSet.then(function() {
							if (self$1._ready === null) self$1._ready = self$1._initDriver();
							return self$1._ready;
						});
						executeTwoCallbacks(promise, callback, callback);
						return promise;
					};
					LocalForage.prototype.setDriver = function setDriver(drivers, callback, errorCallback) {
						...
						function setDriverToConfig() {
							self$1._config.driver = self$1.driver();
						}
						function extendSelfWithDriver(driver) {
							self$1._extend(driver);
							setDriverToConfig();
							self$1._ready = self$1._initStorage(self$1._config);
							return self$1._ready;
						}
						function initDriver(supportedDrivers$1) {
							return function() {
								var currentDriverIndex = 0;
								function driverPromiseLoop() {
									while (currentDriverIndex < supportedDrivers$1.length) {
										var driverName = supportedDrivers$1[currentDriverIndex];
										currentDriverIndex++;
										self$1._dbInfo = null;
										self$1._ready = null;
										return self$1.getDriver(driverName).then(extendSelfWithDriver)["catch"](driverPromiseLoop);
									}
									setDriverToConfig();
									var error = /* @__PURE__ */ new Error("No available storage method found.");
									self$1._driverSet = Promise$1.reject(error);
									return self$1._driverSet;
								}
						...
						this._driverSet = (this._driverSet !== null ? this._driverSet["catch"](function() {
							...
							self$1._dbInfo = null;
							self$1._ready = null;
							return self$1.getDriver(driverName).then(function(driver) {
								self$1._driver = driver._driver;
								setDriverToConfig();
								self$1._wrapLibraryMethodsWithReady();
								self$1._initDriver = initDriver(supportedDrivers);
							});
						})["catch"](function() {
							setDriverToConfig();
							var error = /* @__PURE__ */ new Error("No available storage method found.");
